...
public sealed class PluginAssemblyLoader
{
    ...
    public IAlacrityPlugin Load(PluginPackageDescriptor package)
    {
        ...
        if (!typeof(IAlacrityPlugin).IsAssignableFrom(type)) throw new InvalidOperationException("The manifest-declared entry type does not implement IAlacrityPlugin.");
        return (IAlacrityPlugin)Activator.CreateInstance(type)!;
    }
}
