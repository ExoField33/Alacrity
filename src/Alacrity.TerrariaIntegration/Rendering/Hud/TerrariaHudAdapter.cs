...
internal sealed class TerrariaHudAdapter : IPluginHudRenderer, IPluginHudRenderTransaction
{
    ...
    public void Render(PluginId owner, PluginHudWidgetDescriptor descriptor, Action<IPluginHudCanvas, PluginHudFrame> draw, PluginHudFrame frame)
    {
        canvas.Bind(owner);
        draw(canvas, frame);
        // Flush only after a widget completed successfully. This preserves widget order and keeps
        // commands from a later failed widget out of an earlier widget's avatar layer.
...
internal sealed class TerrariaHudCanvas : IPluginHudCanvas
{
    ...
    internal void FlushPlayerAvatars()
    {
        ...
        for (int index = 0; index < pendingAvatars.Count; index++)
        {
            ...
            // The native 1.4.5.6 head renderer writes through Main.spriteBatch. Rejecting a
            // mismatched adapter batch is safer than allowing one HUD widget to poison another.
            TerrariaAvatarRenderBoundary.TryDrawUiIsolated(spriteBatch, () =>
