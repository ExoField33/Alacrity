...
public sealed class ClientManifestIntegrityTests : IDisposable
{
    ...
    public void ValidatesTheCompleteDeployedRuntimeSet()
    {
        ...
        WriteManifest("3|2|2|1.4.5.6", Hash(executable), "bin/Alacrity.PluginUiCoreBridge.dll", Hash(runtime));

        Assert.True(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out string diagnostic), diagnostic);

        File.WriteAllText(runtime, "tampered");
        Assert.False(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out diagnostic));
    ...
    {
        Assert.False(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out _));

        Directory.CreateDirectory(directory);
        WriteManifest("1|2|2|1.4.5.6", "00", "bin/missing.dll", "00");
        Assert.False(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out string diagnostic));
    ...
    public void RejectsMalformedAndEscapingRuntimeEntries()
    {
        ...
        string executable = Path.Combine(directory, "Alacrity.exe");
        File.WriteAllText(executable, "client");
        WriteManifest("3|2|2|1.4.5.6", Hash(executable), "../outside.dll", "00");
        Assert.False(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out diagnostic));
    ...
    public void DistinguishesExecutableAndRequiredRuntimeIntegrityFailures()
    {
        ...
        string executable = Path.Combine(directory, "Alacrity.exe");
        File.WriteAllText(executable, "client");
        WriteManifest("3|2|2|1.4.5.6", "00", "bin/Alacrity.PluginUiCoreBridge.dll", "00");
        Assert.False(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out string diagnostic));
        Assert.Contains("Alacrity.exe failed", diagnostic, StringComparison.OrdinalIgnoreCase);

        WriteManifest("3|2|2|1.4.5.6", Hash(executable), "bin/Alacrity.PluginUiCoreBridge.dll", "00");
        Assert.False(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out diagnostic));
        Assert.Contains("Alacrity.PluginUiCoreBridge.dll", diagnostic, StringComparison.OrdinalIgnoreCase);

        WriteManifest("3|2|2|1.4.5.6", Hash(executable), "Alacrity.Core.dll", "00");
        Assert.False(ClientManifestIntegrity.TryValidate(directory, "3|2|2|1.4.5.6", out diagnostic));
